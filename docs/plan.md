...
| 2026-08-20 | Folded `/sessions` into `/learn`: both were self-paced watch surfaces (structured courses vs. one-off topic-tagged sessions) competing for the same nav slot. `/learn` now renders courses + a `SessionsFilterGrid` sessions section on one page; `/sessions` (index) 302s to `/learn`; `/sessions/[slug]` detail pages and their canonical URLs are unchanged (still indexed, still carry `VideoObject` JSON-LD). Nav/footer/sitemap/`llms.txt` updated to drop the standalone "Sessions" entry. `/workshops` (B2B booking) was left untouched — it's a different job (book, not watch). |
